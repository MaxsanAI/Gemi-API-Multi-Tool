...
export default function FunctionCalling() {
  ...
  const handleSendMessage = async () => {
    if (prompt.trim()) {
      ...
      try {
        const response = await geminiService.runCalculator(prompt);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Function Calling: Calculator</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid gap-4">
          <p className="text-sm text-muted-foreground">Ask a math question, and Gemini will use a calculator tool to solve it. Try: "What is (5*8) + 10?"</p>
